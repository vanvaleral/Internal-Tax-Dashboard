...
function mergeMonthlyPayload(currentPayload: unknown, incomingPayload: unknown, allowed: Set<string> | null) {
  ...
  for (const [period, rows] of Object.entries(incoming)) {
    ...
    const incomingRows = rows.filter((row) => row && typeof row === "object" && allowed.has(clientIdentifier(row as Record<string, unknown>)));
    const existingRows = Array.isArray(current[period]) ? current[period] : [];
    const retainedRows = existingRows.filter((row) => !row || typeof row !== "object" || !allowed.has(clientIdentifier(row as Record<string, unknown>)));
...
export async function PUT(request: Request) {
  ...
  if (scope === "monthly_compliance") payload = mergeMonthlyPayload(current?.payload, payload, allowed);
